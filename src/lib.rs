...
impl SidFile {
    ...
    fn get_file_type<R: Read>(reader: &mut R) -> Result<Type, std::io::Error> {
    ...
    fn get_version<R: Read>(reader: &mut R, file_type: &Type) -> Result<Version, std::io::Error> {
    ...
    fn get_data_offset<R: Read>(reader: &mut R, version: &Version) -> Result<u16, std::io::Error> {
    ...
    fn get_load_address<R: Read>(reader: &mut R) -> Result<u16, std::io::Error> {
        let load_address = reader.read_u16::<BigEndian>()?;
        Ok(load_address)
    }

    // but why?
    fn get_real_load_address<R: Read>(reader: &mut R) -> Result<u16, std::io::Error> {
        let load_address: u16 = reader.read_u16::<LittleEndian>()?;
        Ok(load_address)
    }

    fn get_play_address<R: Read>(reader: &mut R, file_type: &Type) -> Result<u16, std::io::Error> {
    ...
    fn get_init_address<R: Read>(reader: &mut R, file_type: &Type) -> Result<u16, std::io::Error> {
    ...
    fn get_songs<R: Read>(reader: &mut R) -> Result<u16, std::io::Error> {
    ...
    fn get_start_song<R: Read>(reader: &mut R, songs: u16) -> Result<u16, std::io::Error> {
    ...
    fn get_speed<R: Read>(reader: &mut R, file_type: &Type) -> Result<u32, std::io::Error> {
    ...
    fn get_name<R: Read>(reader: &mut R) -> Result<String, std::io::Error> {
        let mut name = [0u8; 32];
    ...
    fn get_author<R: Read>(reader: &mut R) -> Result<String, std::io::Error> {
    ...
    fn get_released<R: Read>(reader: &mut R) -> Result<String, std::io::Error> {
    ...
    fn get_flags<R: Read>(reader: &mut R) -> Result<Flags, std::io::Error> {
    ...
    fn get_start_page<R: Read>(reader: &mut R) -> Result<u8, std::io::Error> {
        let start_page = reader.read_u8()?;
        Ok(start_page)
    }

    fn get_page_length<R: Read>(reader: &mut R) -> Result<u8, std::io::Error> {
        let page_length = reader.read_u8()?;
        Ok(page_length)
    }

    fn get_sid_address<R: Read>(reader: &mut R) -> Result<u8, std::io::Error> {
        let sid_address = reader.read_u8()?;
        Ok(sid_address)
    }

    fn get_data<R: Read>(reader: &mut R) -> Result<Vec<u8>, std::io::Error> {
        let mut data = vec![];
